...
- **Decoupled Architecture:** Business models and types (like `HistoryItem` and `NutritionFactsData`) are cleanly separated into the `frontend.models` package, while utility classes (such as `JsonUtils` and `IndexedDBUtils`) are separated into the `frontend.utils` package, keeping the main `App.scala` small and focused purely on top-level state coordination.
