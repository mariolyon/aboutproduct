...
- **webapp** -- Scala.js. Laminar-based UI that initially displays "Hello World", provides a text input for a name, and on submit calls the backend greet endpoint. The greeting concatenation happens entirely on the backend; the frontend displays the response as-is.
